...
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY!; // Must use service role to bypass RLS in webhook
...
export async function POST(req: Request) {
  try {
    ...
    const messageData = data?.message || data; // Evolution v2 structure varies slightly sometimes
    ...
    if (!convo) {
      const { data: newConvo, error: errCv } = await supabase
        .from('conversations')
        .insert({
          account_id: accountId,
          contact_id: contactId,
    ...
    await supabase.from('conversations').update({
      ...
      last_message_preview: textContent.slice(0, 60),
      unread_count: convo.unread_count ? convo.unread_count + 1 : 1
    }).eq('id', convo.id);
